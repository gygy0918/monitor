<template>
    <div class="table">
        ...
        <div class="handle-box">
            ...
            <div style="display: inline-block;">
                <el-button type="primary" round @click="addOutwarehouse" >申请出库</el-button>
            </div>
            <el-button type="primary" icon="search" @click="out">出库进度</el-button>
        </div>
...
//            this.getData();
            this.$ajax(
                {
                    method: 'get', //请求方式
                    url: 'http://10.103.249.191:8080/warehouseOutApply/pageByManage',
                    params:{
                        page:1,
                        size:5,
        ...
        methods: {
            ...
            handleEdit(index, row) {
                ...
                this.$ajax(
                    {
                        method: 'post', //请求方式
                        url: 'http://10.103.249.191:8080/warehouseOut',
                        data:row,
                        headers:{"Authorization":localStorage.getItem('token')},
                    }).then((res)=>{
                    this.$message('已同意成功！');
